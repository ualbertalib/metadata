...
import json
from website.tools import PrintException
        ...
        try:
            ...
            if 'records' in viafP_result['searchRetrieveResponse'].keys():
                for records in viafP_result['searchRetrieveResponse']['records']:
                    ...
                    '''if 'titles' in records['record']['recordData']:
                        if records['record']['recordData']['titles'] != null:
                            for work in records['record']['recordData']['titles']['work']:
                    ...
                    if isinstance(records['record']['recordData']['mainHeadings']['data'], list):
                        for text in records['record']['recordData']['mainHeadings']['data']:
                            ...
                            if scoreVP > self.th:
                                '''for ti in titles:
                                    scoreTitle = fuzz.token_sort_ratio(ti, self.work_title)
...
class Results():
    def __init__(self, results, type, log_file):
        ...
        try:
            ...
            for itr in self.results:
                ...
                if scoreVF[0] != "temp" or scoreLC[0] != "temp":                   
                    ...
                    if scoreLC[0] != "temp":
                        self.maxs['LC'] = scoreLC
                    if scoreVF[0] != "temp":
                        self.maxs['VIAF'] = scoreVF                  
        except:
