...
<<<<<<< HEAD
		perm_response = requests.get(perm_q, headers=headers, auth=('fedoraAdmin', 'HN4di2015')).text
=======
		perm_response = requests.get(perm_q, headers=headers, auth=(user, passwd)).text
>>>>>>> master
		with open('Thesis_deposit/perms/' + perm_id + '.nt', 'w') as perm_obj:
...
<<<<<<< HEAD
	response = requests.get(object_q, headers=headers, auth=('fedoraAdmin', 'HN4di2015')).text
=======
	response = requests.get(object_q, headers=headers, auth=(user, passwd)).text
>>>>>>> master
	with open('Thesis_deposit/obj/' + i + '.nt', "w") as output:
		output.write(response)
		output.close()
	with open('Thesis_deposit/obj/' + i + '.nt', "r") as inpu:
		for line in inpu:
			if re.search('hasEmbargo', line):
				embargo_q = line.replace('hydraacl:hasEmbargo <', '').replace('> ;', '').replace('\n', '').lstrip()
				id = embargo_q.split('/')[-1]
<<<<<<< HEAD
				embargo_response = requests.get(embargo_q, headers=headers, auth=('fedoraAdmin', 'HN4di2015')).text
=======
				embargo_response = requests.get(embargo_q, headers=headers, auth=(user, passwd)).text
>>>>>>> master
				with open('Thesis_deposit/embargo/' + id + '.nt', 'w') as embargo_obj:
