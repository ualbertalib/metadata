...
class Query(object):
    ...
    def writeQueries(self):
        for query in self.queries:
            self.queries[query] = self.queries[query].replace('\n', ' ')
...
class Collection(Query):
    ...
    def generateQueries(self):
        for where in self.where:
            ...
            self.queries['collection'] = """{0} {1} }} {2} . BIND(URI(replace(str(?resource), 'http://gillingham.library.ualberta.ca:8080/fedora/rest/prod/', 'http://uat.library.ualberta.ca:8080/fcrepo/rest/uat/')) AS ?jupiterResource)}}""".format(self.prefixes, construct, where)
        self.writeQueries()
    ...
    def generateQueries(self):
        for where in self.where:
            ...
            for pair in self.mapping:
                construct = "{0} ; <{1}> ?{2}".format(construct, pair[0], re.sub(r'[0-9]+', '', pair[0].split('/')[-1].replace('#', '').replace('-', '')))
                where = " {0} . OPTIONAL {{ ?resource <{1}> ?{2} . FILTER (str(?{3})!='') }}".format(where, pair[1], re.sub(r'[0-9]+', '', pair[0].split('/')[-1].replace('#', '').replace('-', '')), re.sub(r'[0-9]+', '', pair[0].split('/')[-1].replace('#', '').replace('-', '')))
            self.queries['community'] = """{0} {1} }} {2} . BIND(URI(replace(str(?resource), 'http://gillingham.library.ualberta.ca:8080/fedora/rest/prod/', 'http://uat.library.ualberta.ca:8080/fcrepo/rest/uat/')) AS ?jupiterResource)}}""".format(self.prefixes, construct, where)
        self.writeQueries()
...
class Generic(Query):
    def __init__(self, sparqlData):
        ...
        self.construct = """CONSTRUCT {
            ...
            dcterm:accessRights ?visibility; rdf:type works:Work; rdf:type pcdm:Object ;
            bibo:owner ?owner ;
...
class characterizationFile(File):
    def __init__(self, sparqlData):
        ...
        self.construct = """CONSTRUCT {
            ...
            rdf:type fedora:Resource ;
            rdf:type pcdm:File; pcdm:fileOf ?fileset; iana:describedby ?fcr ;
            ?predicate ?object
        }"""
    ...
    def generateQueries(self):
        ...
        for group in self.splitBy.keys():
            self.queries[group] = """{0} {1} {2} strStarts(str(?resource), '{3}') ) .
                ...
                BIND(URI(replace(str(?resource), 'http://gillingham.library.ualberta.ca:8080/fedora/rest/prod/', 'http://uat.library.ualberta.ca:8080/fcrepo/rest/uat/')) AS ?jupiterResource) .
                BIND(URI(CONCAT(str(?jupiterResource), '/file')) AS ?file) .
                BIND(URI(REPLACE(str(?jupiterResource), '/characterization', '/fileset')) AS ?fileset) .
                BIND(URI(CONCAT(str(?jupiterResource), '/file/fcr:fixity')) AS ?fixity) .
                BIND(URI(CONCAT(str(?jupiterResource), '/file/fcr:metadata')) AS ?fcr)
            }}""".format(self.prefixes, self.construct, self.where, self.splitBy[group])
        self.writeQueries()
...
class characterizationFileset(Fileset):
    def __init__(self, sparqlData):
        ...
        super().__init__(self.sparqlData, self.objectType, self.filterType)
        self.construct = """CONSTRUCT {
            ?fileset rdf:type fedora:NonRdfSourceDescription;
            rdf:type works:Fileset ;
            rdf:type ual:characterization ;
            pcdm:hasFile ?file ;
            pcdm:memberOf ?parent ;
            iana:describes ?file ;
            ?predicate ?object
        }"""
...
class contentFileset(Fileset):
    def __init__(self, sparqlData):
        ...
        self.construct = """CONSTRUCT {
            ...
            rdf:type fedora:Resource ;
            rdf:type pcdm:Object ;
            rdf:type works:FileSet; rdf:type ldp:Container ;
            rdf:type ldp:RDFSource ;
            pcdm:hasFile ?file ;
            pcdm:memberOf ?parent ;
            ?predicate ?object
