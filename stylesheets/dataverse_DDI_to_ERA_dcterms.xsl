...
<xsl:stylesheet 
    ...
    <xsl:template match="codeBook">
        <xsl:element name="ddi_to_dcterms" inherit-namespaces="no">
            ...
            <!-- normalize-space() function is not applied to the description because Dataverse allows html formatting in this field -->
            <!-- dataKind is included in dcterms:description, rather than dcterms:type because it is a free text field -->
            <!-- ddi:abstract, labelled 'description' in the metadata entry form, is a repeatable field in Dataverse -->
            ...
            <xsl:if test="stdyDscr/stdyInfo/sumDscr/timePrd[@event='start'] or stdyDscr/stdyInfo/sumDscr/timePrd[@event='end']">
