...
def merger():
    source = 'BIBFRAME'
    output = 'Processing/Merged-file_%s.xml' %(datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'))
    with open(output, "w+") as merged_file:
            merged_file.write('<root>')
            for files in [files for files in listdir(source) if isfile(join(source, files))]:
                file = 'BIBFRAME/%s' %(files)
