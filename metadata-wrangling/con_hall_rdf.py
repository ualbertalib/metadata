...
def create_triples():
    ...
    # establish a library of resources that have already been declared in the graph
    # so duplicate resources are not created
    id_lib = { 'CompositionNum': [], 'ComposerURL': [], 'PerformerNum': [], 'PerformanceNum': [], 'RecitalNum': [], 'PerformanceRole': [], 'InstrumentURL': [] }
    ...
    # process one line of the combined csv at a time
        for record in records:       
            ...
            for entity in id_lib.keys():
                
                cell_value = record[entity]
               
                # check if this entity has been processed already and, if not, add it to the graph
                # then add it to the list of already processed entities
                 
                if cell_value not in id_lib[entity]:
                    
                    id_lib[entity].append(cell_value)        
